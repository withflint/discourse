...
class Jobs::NotifyReviewable < ::Jobs::Base
  ...
  def execute(args)
    ...
    if SiteSetting.enable_revamped_user_menu
      ...
      if reviewable.reviewable_by_moderator?
        moderators = User.real.moderators.where("id NOT IN (?)", @contacted)
        notify_users(moderators, all_updates[:moderators])
      end
  ...
  # remove this method with the enable_revamped_user_menu setting
